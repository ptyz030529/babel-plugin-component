            ...
            if (!selectedMethods[methodName]) {
                ...
                if (styleLibraryName) {
                    ...
                    if (libraryObjs[methodName]) {
                        ...
                        themePath = '' + cachePath[libraryName] + (_root || '/index') + '.css';
                        cache[libraryName] = 1;
                    } else {
                        if (cache[libraryName] !== 1) {
                            themePath = cachePath[libraryName] + '/' + camel2Dash(methodName) + '.scss';
                            cache[libraryName] = 2;
                        }
                    }
                    if (fs.existsSync(themePath)) {
                        file.addImport(themePath, 'default');
                    }
                }
